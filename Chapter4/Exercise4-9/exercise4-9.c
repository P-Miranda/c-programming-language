...
/* Out `getch` and `ungetch` do not handle a pushed-back `EOF` correctly. Decide
 * what their properties outght to be if an `EOF` is pushed back, then implement
 * your design. */

/* buf needs to be int to be able to store EOF */
...
int buf; /* buffer for ungetch only a single char, int to handle EOF */
